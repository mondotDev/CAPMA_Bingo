...
export default function AttendeePage() {
  ...
  const staffPreviewActive = useMemo(
    () => !publicAccessOpen && !adminLoading && isAdmin,
    [publicAccessOpen, adminLoading, isAdmin],
  );
  const attendeeAccessOpen = useMemo(
    () => isAttendeeBingoAccessible(currentTime, !adminLoading && isAdmin),
    [currentTime, adminLoading, isAdmin],
  );
  ...
  useEffect(() => {
    console.info("[attendee-access] auth state", {
      ...
      publicAccessOpen,
      staffPreviewAllowed: !adminLoading && isAdmin,
      attendeeAccessOpen,
      currentTimeIso: currentTime.toISOString(),
      openTime: ATTENDEE_BINGO_OPEN_LABEL,
    });
  }, [attendeeAccessOpen, currentTime, adminLoading, isAdmin, publicAccessOpen, user?.isAnonymous]);
  ...
  }, [attendeeAccessOpen, authError, authReady, user?.uid]);
  ...
  if (loading || (!publicAccessOpen && adminLoading)) {
    return (
        ...
        <main className="app-shell">
          <section className="surface-card prelaunch-card">
            ...
            <div className="prelaunch-actions">
              <Link
                className="button-primary prelaunch-link"
                to="/admin-login?preview=1"
              >
                CAPMA Staff Preview
              </Link>
